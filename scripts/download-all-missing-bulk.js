...
const MIN_VALID_CACHE_BYTES = 128 * 1024;
      ...
      if (active < concurrency && idx < datasets.length) {
        ...
        if (fs.existsSync(outFile)) {
          ...
          if (size >= MIN_VALID_CACHE_BYTES) {
            ...
            console.log(`♻️  ${ds.name} cache too small (${(size / 1024).toFixed(1)}KB), re-downloading...`);
          }
        }
...
async function checkNursingHomes() {
  if (fs.existsSync(NURSING_HOMES_CSV)) {
    ...
    let lines = '?';
    try {
      const content = fs.readFileSync(NURSING_HOMES_CSV, 'utf8');
