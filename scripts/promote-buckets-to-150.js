...
(async()=>{
  ...
  try {
    const before = await client.query(`select count(distinct database_name)::int as buckets from environmental_sites`);
    console.log('before_buckets=' + before.rows[0].buckets);
    ...
    for (const [fromDb, cls, newDb] of classMappings) {
      const r = await client.query(
        `update environmental_sites set database_name=$3 where database_name=$1 and class_code=$2 and database_name<>$3`,
        [fromDb, cls, newDb]
      );
      movedClass += r.rowCount || 0;
    }

    let movedRegion = 0;
    for (const [newDb, states] of regionDefs) {
      const r = await client.query(
        `update environmental_sites set database_name=$1 where database_name='RCRA N' and state = any($2::text[]) and database_name<>$1`,
        [newDb, states]
      );
      movedRegion += r.rowCount || 0;
    }

    await client.query('commit');

    const after = await client.query(`select count(distinct database_name)::int as buckets from environmental_sites`);
    console.log('moved_class_rows=' + movedClass);
    console.log('moved_region_rows=' + movedRegion);
    console.log('after_buckets=' + after.rows[0].buckets);
  } catch (e) {
