...
const DATASET_SEEDS = [
  ...
  { name: 'OPENSTREETMAP ROAD NETWORK', category: 'features', country: 'Global' },
...
// Lightweight geospatial fallback records used only when no imported
// geo_points are available in the runtime store.
const FALLBACK_GEO_POINTS = [
  {
    dataset_name: 'EPA ECHO WATER FACILITIES',
    category: 'water',
    country: 'USA',
    name: 'Federal Highway Corridor Facility',
    address: 'Federal Highway, Boynton Beach, FL 33435',
    latitude: 26.5159,
    longitude: -80.0590,
    risk_level: 'MEDIUM',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-boynton-echo-water'
  },
  {
    dataset_name: 'EPA UST REGISTRY',
    category: 'contamination',
    country: 'USA',
    name: 'Boynton Corridor UST Reference Site',
    address: 'Boynton Beach, FL',
    latitude: 26.5410,
    longitude: -80.0698,
    risk_level: 'MEDIUM',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-boynton-ust'
  },
  {
    dataset_name: 'EPA LUST CORRECTIVE ACTION',
    category: 'contamination',
    country: 'USA',
    name: 'Miami LUST Reference Site',
    address: 'Miami, FL',
    latitude: 25.7761,
    longitude: -80.1930,
    risk_level: 'HIGH',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-miami-lust'
  },
  {
    dataset_name: 'EPA ECHO AIR FACILITIES',
    category: 'industrial',
    country: 'USA',
    name: 'Houston Industrial Air Reference Site',
    address: 'Houston, TX',
    latitude: 29.7604,
    longitude: -95.3698,
    risk_level: 'MEDIUM',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-houston-air'
  },
  {
    dataset_name: 'EPA ECHO RCRA FACILITIES',
    category: 'contamination',
    country: 'USA',
    name: 'Los Angeles RCRA Reference Site',
    address: 'Los Angeles, CA',
    latitude: 34.0522,
    longitude: -118.2437,
    risk_level: 'MEDIUM',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-la-rcra'
  },
  {
    dataset_name: 'EPA ECHO WATER FACILITIES',
    category: 'water',
    country: 'USA',
    name: 'New York Water Reference Site',
    address: 'New York, NY',
    latitude: 40.7128,
    longitude: -74.0060,
    risk_level: 'LOW',
    source: 'GeoScope reference fallback',
    source_id: 'fallback-ny-water'
  }
];
...
function ensureStore() {
  ...
  if ((store.geo_points || []).length === 0) {
    for (const row of FALLBACK_GEO_POINTS) {
      const datasetName = String(row.dataset_name || row.dataset || 'Fallback Dataset').trim();
      ...
      store.geo_points.push({
        ...
        site_uid: sanitizeSiteUid(row.source_id) || `fallback-${store.nextGeoPointId}`,
        dataset_id: dataset.id,
        name: row.name || dataset.name,
...
function buildAnalystCorrectionCsv() {
  ensureStore();
  const store = readStore();
  const datasetsById = new Map(store.datasets_master.map((item) => [item.id, item]));
  ...
  for (const row of store.geo_points) {
    if (!Number.isFinite(row.latitude) || !Number.isFinite(row.longitude)) continue;
    const dataset = datasetsById.get(row.dataset_id) || {};
    const sourceId = row.source_id || row.site_uid || '';
    const csvRow = [
      sourceId,
      row.site_uid || '',
      row.id,
...
function applyGeoPointCorrections(rows = [], options = {}) {
  ensureStore();
  const store = readStore();
  const analyst = String(options.analyst || 'analyst').trim() || 'analyst';
  const datasetsById = new Map(store.datasets_master.map((item) => [item.id, item]));

  const byUid = new Map();
  const byId = new Map();
  for (const point of store.geo_points || []) {
    const uid = sanitizeSiteUid(point.site_uid || point.source_id);
    if (uid) byUid.set(uid, point);
    if (Number.isFinite(Number(point.id))) byId.set(Number(point.id), point);
  }
  ...
  const updatedDatasetNames = new Set();
  const updatedRecords = [];

  for (const row of rows) {
    reviewed += 1;

    const rawSourceId = row.source_id || row.site_uid || row.Source_ID || row.SOURCE_ID || '';
    const rawLocalId = row.local_id || row.localId || row.id || row.ID || '';
    const keyUid = sanitizeSiteUid(rawSourceId);
    const keyId = Number(rawLocalId);

    let point = null;
    if (keyUid && byUid.has(keyUid)) {
      point = byUid.get(keyUid);
    } else if (Number.isFinite(keyId) && byId.has(keyId)) {
      point = byId.get(keyId);
    }
    ...
    const lat = toNumber(
      row.corrected_latitude ?? row.latitude ?? row.lat ?? row.Latitude ?? row.LATITUDE
    );
    const lng = toNumber(
      row.corrected_longitude ?? row.longitude ?? row.lng ?? row.lon ?? row.Longitude ?? row.LONGITUDE
    );
    ...
    const oldLat = Number(point.latitude);
    const oldLng = Number(point.longitude);
    const changed = oldLat !== lat || oldLng !== lng;
    ...
    const correctionNote = String(row.correction_note || row.note || row.correction_reason || '').trim();
    point.last_corrected_by = analyst;
    point.last_corrected_at = new Date().toISOString();
    point.last_correction_note = correctionNote || null;
    point.correction_count = Number(point.correction_count || 0) + 1;

    const datasetName = datasetsById.get(point.dataset_id)?.name || 'UNKNOWN DATASET';
    updatedDatasetNames.add(datasetName);
    updatedRecords.push({
      local_id: Number(point.id),
      source_id: point.source_id || point.site_uid || null,
      site_uid: point.site_uid || null,
  ...
  return {
    ...
    invalid_coordinate_rows: invalidCoords,
    updated_dataset_count: updatedDatasetNames.size,
    updated_datasets: Array.from(updatedDatasetNames).sort((a, b) => a.localeCompare(b)),
