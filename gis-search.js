...
const { dataPool: pgPool } = require('./db');
...
function parseMRDSWfsGml(text) {
  const xml = String(text || '');
  if (!xml.includes('<wfs:member>')) return [];

  const rows = [];
  const members = xml.match(/<wfs:member>[\s\S]*?<\/wfs:member>/g) || [];

  for (const member of members) {
    const posMatch = member.match(/<gml:pos>\s*([-+]?\d+(?:\.\d+)?)\s+([-+]?\d+(?:\.\d+)?)\s*<\/gml:pos>/i);
    if (!posMatch) continue;

    const a = Number(posMatch[1]);
    const b = Number(posMatch[2]);
    if (!Number.isFinite(a) || !Number.isFinite(b)) continue;

    let fLat = a;
    let fLng = b;
    if (!(Math.abs(a) <= 90 && Math.abs(b) <= 180) && (Math.abs(b) <= 90 && Math.abs(a) <= 180)) {
      fLat = b;
      fLng = a;
    }

    const siteName = member.match(/<ms:site_name>([\s\S]*?)<\/ms:site_name>/i)?.[1]?.trim() || 'Mine Site';
    const status = member.match(/<ms:dev_stat>([\s\S]*?)<\/ms:dev_stat>/i)?.[1]?.trim() || 'Unknown';
    const depId = member.match(/<ms:dep_id>([\s\S]*?)<\/ms:dep_id>/i)?.[1]?.trim() || null;
    const detailUrl = member.match(/<ms:url>([\s\S]*?)<\/ms:url>/i)?.[1]?.trim() || '';
    const fips = member.match(/<ms:fips_code>([\s\S]*?)<\/ms:fips_code>/i)?.[1]?.trim() || '';
...
async function fetchUSGSNwisRowsWithFallback(west, south, east, north, options = {}) {
  const siteStatusCandidates = Array.isArray(options.siteStatusCandidates) && options.siteStatusCandidates.length > 0
    ? options.siteStatusCandidates
    : ['all'];
  const siteTypeCandidates = Array.isArray(options.siteTypeCandidates)
    ? options.siteTypeCandidates
    : [null];
  ...
  const urls = [];
  for (const status of siteStatusCandidates) {
    for (const siteType of siteTypeCandidates) {
      for (const bboxParam of bboxPairs) {
        const typeClause = siteType ? `&siteType=${encodeURIComponent(siteType)}` : '';
        urls.push(
          `https://waterservices.usgs.gov/nwis/site/?format=rdb&${bboxParam}&siteStatus=${encodeURIComponent(status)}${typeClause}`
        );
      }
    }
  }
  ...
  throw lastError || new Error('No NWIS endpoint returned data');
}
...
function normalizeLegacyStoreRecord(row, lat, lng) {
  ...
  return {
    ...
    class_code: row.class_code || row.class || null,
    class_description: row.class_description || null,
    priority_tier: row.priority_tier || 'standard',
...
async function fetchPostgresSites(lat, lng, radius_m) {
  const sqlPriority = `
    ...
    ORDER BY COALESCE(priority_score, 0) DESC, distance_m ASC
    LIMIT $4
  `;
  ...
  const maxResults = Math.max(100, Number(process.env.GIS_MAX_RESULTS || 10000));

  try {
    let query;
    try {
      query = await pgPool.query(sqlPriority, [lng, lat, radius_m, maxResults]);
    } catch (priorityErr) {
      if (priorityErr && priorityErr.code === '42703') {
        query = await pgPool.query(sqlLegacy, [lng, lat, radius_m, maxResults]);
      } else {
        throw priorityErr;
      }
    }
    ...
    return query.rows.map((r) => ({
      ...
      class_description: r.class_description || null,
      priority_tier: r.priority_tier || 'standard',
      priority_score: Number.isFinite(Number(r.priority_score)) ? Number(r.priority_score) : 0,
...
async function fetchMines(lat, lng, radius_m) {
  const { west, east, south, north } = bboxFromCenter(lat, lng, radius_m);
  try {
    const bboxValue = `${west},${south},${east},${north},EPSG:4326`;
    const typeNames = ['ms:mrds-high', 'ms:mrds-low'];
    const allRows = [];
    ...
    for (const typeName of typeNames) {
      const url =
        ...
        `&bbox=${bboxValue}&count=40`;

      try {
        const xml = await axios.get(url, { timeout: 25000, httpsAgent: GIS_AGENT }).then((r) => r.data);
        const rows = parseMRDSWfsGml(xml);
        if (rows.length > 0) allRows.push(...rows);
      } catch (error) {
        // Continue with fallback layer if one layer fails or has no records for this bbox.
      }
    }
    ...
    return allRows
      ...
      .map((f, i) => {
        ...
        return {
          ...
          database: 'MINES',
          category: 'geology',
          site_name: f.site_name || 'Mine Site',
          address: f.fips_code || '',
          lat: fLat,
          lng: fLng,
          distance_m: haversineMeters(lat, lng, fLat, fLng),
          status: f.dev_stat || 'Unknown',
          source: 'USGS MRDS WFS',
          reference_url: f.url || undefined,
        };
      })
      .filter(
        (f) =>
...
async function fetchUSGSWaterSites(lat, lng, radius_m) {
  try {
    ...
    const rows = await fetchUSGSNwisRowsWithFallback(b.west, b.south, b.east, b.north, {
      siteStatusCandidates: ['active', 'all'],
      siteTypeCandidates: [null],
    });
...
async function fetchOSMFacilities(lat, lng, radius_m, amenityTypes, dbLabel, category) {
  const amenityList = Array.isArray(amenityTypes) ? amenityTypes : [amenityTypes];
  const amenityRegex = amenityList
  ...
  const query = `
    [out:json][timeout:25];
    (
      node(around:${Math.round(radius_m)},${lat},${lng})["amenity"~"^(${amenityRegex})$"];
      way(around:${Math.round(radius_m)},${lat},${lng})["amenity"~"^(${amenityRegex})$"];
      relation(around:${Math.round(radius_m)},${lat},${lng})["amenity"~"^(${amenityRegex})$"];
    );
    out center tags;
  `;
  ...
  for (const endpoint of overpassEndpoints) {
    try {
      data = await axios.post(endpoint, query, {
        headers: {
          'Content-Type': 'text/plain',
          'User-Agent': 'Geoscope GIS Search 1.0 (+https://geoscopesolutions.com)'
        },
  ...
  if (!data) {
    console.warn(`[GIS] OSM ${dbLabel} fetch failed:`, lastError?.message || 'Unknown Overpass error');
    return [];
  }
...
async function fetchNWISStreamflow(lat, lng, radius_m) {
  try {
    ...
    const rows = await fetchUSGSNwisRowsWithFallback(b.west, b.south, b.east, b.north, {
      siteStatusCandidates: ['all', 'active'],
      siteTypeCandidates: ['ST', null],
    });
    ...
    return rows
      ...
      .map((row, i) => {
        ...
        return {
          id: `nwis-site-${row.site_no || i}`,
          database: 'NWIS WATER SITES',
          category: 'hydrology',
          site_name: row.station_nm || 'USGS Water Monitoring Site',
          address: [row.county_cd, row.state_cd].filter(Boolean).join(', '),
          lat: fLat,
          lng: fLng,
          distance_m: haversineMeters(lat, lng, fLat, fLng),
    ...
    console.warn('[GIS] NWIS streamflow failed:', e.message);
    return [];
  }
}
...
async function nearbySearch(lat, lng, radius_m = 1609.344) {
  lat = parseFloat(lat);
  lng = parseFloat(lng);
  ...
  radius_m = Math.min(Math.max(Number(radius_m) || 1609.344, 50), 24140); // 15 mile hard cap
  const configuredSourceTimeout = Number(process.env.GIS_SOURCE_TIMEOUT_MS || 0);
  const sourceTimeoutMs = Number.isFinite(configuredSourceTimeout) ? Math.max(0, configuredSourceTimeout) : 0;
  ...
  const [
    ...
    settleWithin(fetchPostgresSites(lat, lng, radius_m), sourceTimeoutMs, 'PostgreSQL'),
    settleWithin(fetchEchoFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA ECHO'),
    settleWithin(fetchFloodZones(lat, lng), sourceTimeoutMs, 'FEMA Flood'),
    settleWithin(fetchWetlands(lat, lng, radius_m), sourceTimeoutMs, 'USFWS Wetlands'),
    settleWithin(fetchMines(lat, lng, radius_m), sourceTimeoutMs, 'USGS Mines'),
    settleWithin(fetchFarmLanduse(lat, lng, radius_m), sourceTimeoutMs, 'OSM Farm Landuse'),
    settleWithin(fetchTRIFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA TRI'),
    settleWithin(fetchSuperfundSites(lat, lng, radius_m), sourceTimeoutMs, 'EPA Superfund'),
    settleWithin(fetchRMPFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA RMP'),
    settleWithin(fetchFRSFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA FRS'),
    settleWithin(fetchSSURGOSoil(lat, lng), sourceTimeoutMs, 'USDA SSURGO'),
    settleWithin(fetchUSGSEarthquakes(lat, lng, radius_m), sourceTimeoutMs, 'USGS Earthquakes'),
    settleWithin(fetchUSGSWaterSites(lat, lng, radius_m), sourceTimeoutMs, 'USGS Water Sites'),
    settleWithin(fetchCERCLISSites(lat, lng, radius_m), sourceTimeoutMs, 'EPA CERCLIS'),
    settleWithin(fetchNPLSites(lat, lng, radius_m), sourceTimeoutMs, 'EPA NPL'),
    settleWithin(fetchNPLDelisted(lat, lng, radius_m), sourceTimeoutMs, 'EPA NPL Delisted'),
    settleWithin(fetchPCSFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA PCS'),
    settleWithin(fetchPWSSystems(lat, lng, radius_m), sourceTimeoutMs, 'EPA PWS'),
    settleWithin(fetchICISFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA ICIS'),
    settleWithin(fetchSTATSGO(lat, lng, radius_m), sourceTimeoutMs, 'USDA STATSGO'),
    settleWithin(fetchSuperfundProposed(lat, lng, radius_m), sourceTimeoutMs, 'EPA Proposed NPL'),
    settleWithin(fetchSchools(lat, lng, radius_m), sourceTimeoutMs, 'OSM Schools'),
    settleWithin(fetchHospitals(lat, lng, radius_m), sourceTimeoutMs, 'OSM Hospitals'),
    settleWithin(fetchDaycare(lat, lng, radius_m), sourceTimeoutMs, 'OSM Daycare'),
    settleWithin(fetchColleges(lat, lng, radius_m), sourceTimeoutMs, 'OSM Colleges'),
    settleWithin(fetchChurches(lat, lng, radius_m), sourceTimeoutMs, 'OSM Churches'),
    settleWithin(fetchPrisons(lat, lng, radius_m), sourceTimeoutMs, 'OSM Prisons'),
    settleWithin(fetchNursingHomes(lat, lng, radius_m), sourceTimeoutMs, 'OSM Nursing Homes'),
    settleWithin(fetchArenas(lat, lng, radius_m), sourceTimeoutMs, 'OSM Arenas'),
    settleWithin(fetchGovernmentBuildings(lat, lng, radius_m), sourceTimeoutMs, 'OSM Government'),
    settleWithin(fetchAirports(lat, lng, radius_m), sourceTimeoutMs, 'OSM Airports'),
    settleWithin(fetchNWISStreamflow(lat, lng, radius_m), sourceTimeoutMs, 'USGS NWIS'),
    settleWithin(fetchAIRSFacilities(lat, lng, radius_m), sourceTimeoutMs, 'EPA AIRS'),
    settleWithin(fetchHazWasteManifest(lat, lng, radius_m), sourceTimeoutMs, 'EPA HazWaste'),
    settleWithin(fetchUST(lat, lng, radius_m), sourceTimeoutMs, 'EPA UST'),
    settleWithin(fetchLUST(lat, lng, radius_m), sourceTimeoutMs, 'EPA LUST'),
    settleWithin(fetchRMPPSI(lat, lng, radius_m), sourceTimeoutMs, 'EPA RMP PSI'),
    settleWithin(fetchFedBrownfields(lat, lng, radius_m), sourceTimeoutMs, 'EPA Brownfields'),
    settleWithin(fetchFRSACRES(lat, lng, radius_m), sourceTimeoutMs, 'EPA FRS ACRES'),
    settleWithin(fetchFRSCEDRI(lat, lng, radius_m), sourceTimeoutMs, 'EPA FRS CEDRI'),
    settleWithin(fetchFRSNCDB(lat, lng, radius_m), sourceTimeoutMs, 'EPA FRS NCDB'),
  ]);
