$api = 'https://geoscope-api.vercel.app/client-orders'
$orders = @(
...
foreach ($o in $orders) {
  try {
    $resp = Invoke-RestMethod -Uri $api -Method Post -ContentType 'application/json' -Body ($o | ConvertTo-Json -Depth 6) -TimeoutSec 90
    $orderId = $null
    if ($resp.order -and $resp.order.id) { $orderId = $resp.order.id }
    elseif ($resp.persistedOrderId) { $orderId = $resp.persistedOrderId }
